...
const seedOrders: Order[] = [
  { id: 'ORD-001', customerName: 'Ama Mensah', productName: 'Ankara Dress', quantity: 1, total: 350, paymentReceived: 350, outstanding: 0, paymentStatus: 'Paid', status: 'Delivered', createdAt: new Date().toISOString() },
  { id: 'ORD-002', customerName: 'Esi Boateng', productName: 'Two-Piece Set', quantity: 1, total: 420, paymentReceived: 200, outstanding: 220, paymentStatus: 'Partially Paid', status: 'Preparing', createdAt: new Date().toISOString() },
  { id: 'ORD-003', customerName: 'Adwoa Owusu', productName: 'Linen Shirt', quantity: 1, total: 180, paymentReceived: 0, outstanding: 180, paymentStatus: 'Unpaid', status: 'New', createdAt: new Date().toISOString() },
];
...
export default function OrdersPage() {
  ...
  useEffect(() => {
    const stored = window.localStorage.getItem('fashion-seller-orders');
    setOrders(stored ? JSON.parse(stored) : seedOrders);
  }, []);
    ...
    <main className="min-h-screen bg-slate-50">
      <header className="border-b bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 lg:px-8">
          <div><p className="text-sm font-extrabold tracking-wide text-indigo-600">ORDERS</p><h1 className="mt-1 text-2xl font-bold">Sales & Orders</h1></div>
          <Link href="/orders/new" className="rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-bold text-white">+ New Order</Link>
        </div>
      </header>
